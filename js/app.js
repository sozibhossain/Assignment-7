...
const displayProducts = (products) => {
  ...
  products.forEach(product => {
    ...
    div.innerHTML = `<div class="single-product">
      ...
      <button onclick="loadProductByQuantity('${product.description}')" id="details-btn" class="btn btn-danger">Details</button></div>
      `;
...
const searchProduct = () => {
  ...
  searchArea.value= '';
  const url = `https://fakestoreapi.com/products?limit=${searchText}`;
  fetch(url)
  .then(res => res.json())
...
const displaySearchResult = product => {
  const searchResult = document.getElementById('search-area');
  product.forEach(product => {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <button onclick="loadProductByQuantity('${product.description}')" id="details-btn" class="btn btn-danger">Details</button></div>
      `;
    document.getElementById("product-filde").appendChild(div);
  })
}
